...
service tftpd-hpa start || echo "Skipping tftpd-hpa (not available or not needed)"
if [ "$nochrony" != true ]; then
    service chrony start || echo "Skipping chrony (not available or not needed)"
fi
...
service fail2ban restart >> "$log" 2>&1 || echo "fail2ban service not available or not running" >> "$log"
...
# systemctl is not available in Docker; log and skip
echo "Skipping systemctl daemon-reload (not supported in Docker)" >> "$log"

if [ ! "$nofpbx" ] ; then
  echo "Skipping systemctl enable freepbx (not supported in Docker)" >> "$log"
fi
...
# Restart apache2
setCurrentStep "Restarting Apache"
